...
Service Resilience
	...
	3. AZ Resilient: Run from a single AZ. It is possible for hardware to fail in an AZ, and the service to keep running because of redundant equipment, but should not be relied on. Usually prone to failure.
...
One template can create an infinite amount of stacks.
...
Service Control Policies (e.g account permission boundaries - what permissions can be granted in the account)
...
DNS provided by R53
Available on the base IP address of the VPC + 2. For example, if the VPC is 10.0.0.0 then the DNS IP will be 10.0.0.2
...
Route 53 Routing Policies Examples
	...
	• Weighted: Create multiple records of the same name within the hosted zone. For each of those records, you provide a weighted value. The total weight is the same as the weight of all the records of the same name. If all of the parts of the same name are healthy, it will distribute the load based on the weight. If one of them fails its health check, it will be skipped over and over again until a good one gets hit. This can be used for migration to separate servers. Implements active-active failover. For load balancing or testing new software versions.
...
Every item in a table can also have attributes, but they don't have to be the same between values. The only requirement is that every item inside the table has to use the same key structure and it has to have a unique key.
Document
...
Reasons EC2 Database might make sense
	...
	• Architecture AWS don’t provide (e.g. replication/resilience)

Reasons why you really shouldn't run a database on EC2
	• Admin overhead is intense to manage the EC2 host and DBHost.
	• Backup and Disaster Recovery (DR) Management adds complexity.
	• EC2 is single AZ. If the zone fails, access to the database fails.
	• Will miss out on features from AWS DB products.
	• EC2 is ON or OFF, there is no way to scale easily and to do serverless (e.g. keep up with bursts demand).
	• Replication can be tricky to manage on your own.
	• Performance will be slower than other AWS options. AWS invests time into DB optimization & features
...
Relational Database Service (RDS)
	...
	• Handles engines such as MySQL, MariaDB, PostgreSQL, Oracle, even Microsoft SQL.
	• Amazon Aurora. This is so different from normal RDS, it is a separate product.

RDS Database Instance (the basic building block of RDS)
	...
	• The database can be optimized for: db.m5 general, db.r5 (memory optimized), db.t3 burst
	...
	• Billing is per instance and hourly rate for that compute. You are also billed for storage allocated. GB/month metric
	• Access to the database is thru a Security Group
...
RDS Exam Power Up
	...
	• 60-120 seconds failover (HA not Fault-tolerant)
...
RPO - Recovery Point Objective
	...
	• Business usually provides an RPO value (lower value, costs more)
RTO - Recovery Time Objective
	• Time between the disaster recovery event and full recovery.
	• Influenced by process, staff, tech and documentation.
	• Business usually provides an RTO value (lower value, costs more)
	

RDS Backups
	• First snap is full copy of the data used on the RDS volume. From then on, the snapshots are incremental and only store the changes in data.
	...
	• Automatic cleanups can be anywhere from 0 to 35 days. This means you can restore to any point in that time frame. This will use both the snapshots and the translation logs. 0 days means no automatic backup configured.
...
READ performance improvements
	...
	• Can provide global performance improvements (e.g. scale-out reads).
	• Provides global resilience by using cross region replication.
	• They don't improve RTO
Read Replicas provide near 0 RPO (availability improvements)
	• If the primary instance fails, you can promote a read-replica to take over.
	• Once it is promoted, it allows for read and write (e.g. read only - until promoted)
...
Aurora Restore, Clone and Backtrack
	• Backups in Aurora work in the same way as RDS. Restores creates a brand new cluster.
	• Backtrack must be enabled on a per cluster basis. This allows you to roll back your data base to a previous point in time (in-place rewinds). This helps for data corruption.
	• You can adjust the window backtrack will work for.
	• Fast clones make a new database much faster than copying all the data. It references the original storage and only write the differences between the two. It uses a tiny amount of storage and only stores data that's changed in the clone or changed in the original after you make the clone - copy-on-write.
...
Introduces the idea of secondary regions with up to 16 read only replicas. Replication from primary region to secondary regions happens at the storage layer and typically occurs within one second.
	...
	• There is ~1s or less replication between regions (occurs at the storage layer).
...
Aurora Multi-Master Writes
	...
	• This also ensures storage is updated on in-memory caches of other nodes.
...
Database Migration Service (DMS)
	...
	• Instead Full Load + CDC (Change data capture) allows for a full load transfer to occur and it monitors any changes that happens during this time. Any of the captured changes can be applied to the target.
	• CDC only migration is good if you have a vendor solution that works quickly and use DMS to capture only changes need to be captured.
...
EFS Exam PowerUp
	...
	• Two performance modes: 
		...
		○ Max I/O performance mode can scale to higher levels of aggregate t-put and IOPS but it does have increased latencies. For highly parallel applications.
	• Two t-put modes: 
...
Cross zone load balancing
	...
	• ALB can support a wide array of targets (services that ALBs can distribute connections to). Targets are grouped within target groups and an individual target can be a member of multiple groups. It's the groups which ALBs distribute connections to. You could create rules to direct traffic to different Target Groups based on their DNS.
...
Scheduled Scaling - time based adjustments Dynamic Scaling
...
Cooldown Period is how long to wait at the end of a scaling action, before scaling again because there is a minimum billable duration for an EC2 instance. Currently this is 300 seconds.
...
Simple Instance Recovery - Launch Template ASG with minimum/desired/max of 1  in different Azs. Cheap and simple HA.
...
Network Load Balancer (NLB) - much faster in latency
...
Connection Stickiness
	...
	• This could cause backend unevenness because one user will always be forced to the same server no matter what the distributed load is. Applications should be designed to hold session stickiness somewhere other than EC2 (e.g. stateless services like DynamoDB).
...
Event Driven Architecture
	• Event producers 
		○ Interact with customers, part of EC2 or systems monitoring components.
	...
	• Event router is needed for event driven architecture that also manages an event bus (a constant flow of information).
	• Only consumes resources while handling events.
	• No constant running or waiting for things
	• Producers generate events when something happens
	• Mature event-driven  architecture only consumes resources while handling events (e.g. serverless)
...
CloudWatch Events and EventBridge
	• Delivers near real time stream of system events that describe changes in AWS products and services. 
	• EventBridge will replace CW Events. 
	• EventBridge can also handle events from third parties (as well as custom applications). Both share the same underlying architecture. AWS is now encouraging a migration to EB.
...
Great during an architecture evolution because the endpoints don't change.
	...
	2. Using API gateway allows the business to evolve along the way slowly. This might move some of the data to fargate and aurora architecture (microservice).
	3. Move to a full serverless architecture with DynamoDB (serverless).
...
Serverless
	...
	• Aim is to consume as a service whatever you can, code as little as possible, and use function as a service (FaaS) for any general purpose compute needs, and then use all of those building blocks together to create your application.
...
A user wants to upload videos to a website for transcoding.
	...
	2. Third party auth provider, Google IDP in this case, authenticates via token.
...
Simple Notification Service (SNS)
	...
	• Coordinates the sending and delivering of messages up to 256KB in size. 
	...
	• Subscribers receive all of the messages sent to the Topic. 
		...
		○ Filters can be applied to limit messages sent to subscribers (e.g. by relevance)
	...
	• Used across AWS for notifications - for example, CloudWatch and CloudFormation
Offers:
...
AWS Step Functions
	...
	• A state machine is a workflow. It has a start point, end point and in between there are states. States are things inside a State Machine which can do things. States can do things, and take in data, modify data, and output data.
...
States are the things inside a workflow, the things which occur. These states are available.
	...
	• Choice 
		○ different path is determined based on an input
	• Parallel 
		○ will create parallel branches based on a choice
	• Map 
		○ accepts a list of things (e.g. orders)
...
Access to a queue is based on identity policies or a queue policy. Queue policies only can allow access from an outside account (e.g. resource policy)

Kinesis (great for real-time Analytics and Dashboards)
	...
	• Multiple consumers can access data from that moving window (e.g. at different granularity). 
...
Kinesis data records (1MB) are stored across shards and are the blocks of data for a stream. Data is stored thru this records.
Kinesis Data Firehose connects to a Kinesis stream. It can move the data from a stream onto S3 or another service.

SQS vs Kinesis
Kinesis
	• Large throughput or large numbers of devices (e.g. at scale)
...
SQS
	• 1 thing sending messages to the queue ( e.g. 1 production group, 1 consumption group)
	• One consumption group from that tier
	• Allow for decoupling and  async communications
...
Architecture Basics
	...
	• Edge locations - local infrastructure which hosts a cache of your data. 
		...
		○ Normally 90% storage with some small compute (e.g. mainly used for content services)
	• Regional Edge Cache 
		...
		○ Provides 2nd layer of caching.
	• How online content are accessed
		1. Checks Edge
		2. Checks Regional Edge
		3. Origin Fetch
Caching Optimization
Parameters can be passed on the url such as query string parameters. An example is ?language=en and ?language=es
...
AWS Certificate Manager (ACM)
	• HTTP lacks encryption and is insecure (e.g. no server identity authentication)
	...
	• For supported AWS services ONLY (CloudFront and ALB, NOT EC2)
...
Lambda@Edge
	• Cloudfront feature : Allow you to run lightweight Lamda at edge locations
	...
	Use Cases
		...
		○ Content by Country - Origin Requests
...
AWS Global Accelerator
	...
	KEY CONCEPTS
		...
		○ Connections enter at edge..using anycast IPSs
...
VPC Flow Logs
	...
	• Flow log inheritance is downwards starting at the VPC (e.g. VPC > subnet > interface).
...
VPC Endpoints (Gateway)
	...
	• S3 buckets can be set to private only by allowing access ONLY from a gateway endpoint. For anything else, the implicit deny will apply. Prevent leaky buckets !
...
VPC Endpoints (Interface)
	...
	• Behind the scenes, it uses PrivateLink (access services hosted on AWS easily and securely by keeping network traffic with the AWS network).
...
Storage Gateway (hybrid storage virtual appliance) - extension, migration & backups
	• Hybrid Storage Virtual Application (On-premise) 
		○ Can be run inside AWS as part of certain disaster recovery scenarios
		○ Allows for migration of existing infrastructure into AWS slowly (e.g. long term trickle migration) .
		○ Extension of File & Volume Storage into AWS
		○ 3 Main Modes
			§ Tape Gateway (VTL) Mode 
				□ For migrating an existing tape backup architecture
				□ Pretends to be a iSCSI tape library, changer and drive
				□ Virtual Tapes are stored on S3 & Glacier (when ejected and archived)
				□ Virtual tape 100 GiB => 5 TiB
				□ 1 PB total storage across 1500 virtual tapes
			§ File Gateway Mode (using SMB and NFS protocols) 
				□ File Storage Backed as S3 Objects
			§ Volume Mode (EBS, but running on-prem)
				□ Block storage backed by S3 and EBS Snapshots
					® (Gateway Stored) - extensions into AWS
						◊ Great for disaster recovery
						◊ Can create up to 32 volumes (of up to 16Tb each)
						◊ Data is kept locally (backup data is then asynchronously replicated to AWS)
						◊ Awesome for migrations and DR (AWS side creates EBS snapshots from backup data)
					® (Gateway Cached) 
						...
						◊ Local storage is only for frequently-accessed data
...
Snowball
	...
	• 1 Gbps (RJ45 1GBase-TX) or 10 (LS/SR) Gbps connection
	• 50TB or 80TB Capacity. 
		○ 10TB to 10PB of data is economical range (can order multiple devices even to multiple premises)
...
Snowball Edge
	...
	• Three types of Snowball Edge 
		...
		○ Compute with GPU 
			§ Same as compute, but with GPU (for scientific analysis, modelling)
...
AWS DataSync
	...
	• Each agent can handle 10Gbps each (up to 100 TB per day).
	...
	• Pay as you use product (per Gb cost for data moved)
AWS DataSync Components
	...
	• Agent 
		○ software to read and write to on prem using as NFS or SMB
...
Words to look for
	• VSS: User Driven Restores (allows users to do file/folder restores)
	• Native File System (NFS) accessible over SMB
	• Uses Windows permissions model
	• Product supports DFS (scale out file share structure)
...
FSx for Lustre
	...
	• For Machine Learning, Big Data, Financial Modelling
	• 100's GB/s throughput & sub milliseconds latency
	• Deployment types - Persistent or Scratch
		○ Scratch - highly optimized for short term (no replication and fast)
		○ Persistent - longer term, HA (in one AZ), self-healing
		○ Accessible over VPN or Direct Connect
	• Scratch is designed for pure performance
		○ For short term or temp workloads
		○ No HA…no replication
		○ Larger file systems means more servers, more disks and more chances of failure!
	• Persistent type has replication within one AZ only
	• Auto-heals when hardware failure occurs
	• You can backup to S3 with both ! (Manual or Automatic 0-35 day retention)
	• Lustre file system > where the processing of data actually occurs
...
NoSQL Database as a Service (DBaaS)
	...
	• Not like RDS which is a Database Server as a Product (it is a Database Table as a Service product)
		○ This is only the database.
	• Capacity can be provisioned or use on-demand mode (i.e. set and forget)
...
Dynamo DB Tables
	...
	• Two types of primary key: 
		○ Simple (Partition)
		○ Composite (Partition and Sort) - PK & SK
...
Point-in-time Recovery (PITR): Must be enabled on each table and is off by default. This allows continuous record of changes for 35 days to allow you to replay any point in that window to a 1 second granularity.
...
DynamoDB Consistency Model
	...
	• A strongly consistent read always uses the leader node and is less scalable (more expensive)
...
There are four view types that it can be configured with:
	• KEYS_ONLY : only record the partition key (and optionally the sort key)
...
ElasticCache (increase database & web application performance)
	...
	• 2 Cache engines:
		○ Memcached
			...
			§ No replication
			§ Multiple Noes (Sharding)
			§ Multi-thread by design
		○ Redis
			§ Advance structures (e.g. lists, sets, sorted sets, bit arrays)
			§ Multi-AZ
			§ Replication (Scale Reads)
			§ Backup & Restore
			§ Transactions (treat multiple operations as one)
